...
public class BeatBox{
	...
	public void buildMIDIButtons(){
		...
		midiButtonsBox.add(saveButton);
		midiButtonsBox.add(restoreButton);
	...
	public class SaveActionListener implements ActionListener{
		public void actionPerformed(ActionEvent event){
			...
			savePattern(saveFile.getSelectedFile());
		}
	}

	public class RestoreActionListener implements ActionListener{
		public void actionPerformed(ActionEvent event){
			JFileChooser restoreFile = new JFileChooser();
			restoreFile.showOpenDialog(frame);
			File newFile = restoreFile.getSelectedFile();
			if(newFile != null)
			restorePattern(newFile);
		}
	}
	...
	public void savePattern(File file){
		try{
			boolean[] chkPattern = new boolean[256];
			int i=0;
			for(JCheckBox check :checkBoxList){
				chkPattern[i]=check.isSelected();
				i++;
			}
			FileOutputStream fos = new FileOutputStream(file);
			ObjectOutputStream oos = new ObjectOutputStream(fos);
			oos.writeObject(chkPattern);
			oos.close();
		} catch(Exception e){
			e.printStackTrace();
		}
	...
	public void restorePattern(File file){
		try{
			System.out.println("New File");
			FileInputStream fis = new FileInputStream(file);
			ObjectInputStream ois = new ObjectInputStream(fis);
			boolean[] chkPattern = (boolean[])ois.readObject();
			System.out.println(chkPattern.length);
			int i=0;
			for(JCheckBox checkBox :checkBoxList){
				System.out.println(chkPattern[i]);
				checkBox.setSelected(chkPattern[i]);
				i++;
			}
			ois.close();
